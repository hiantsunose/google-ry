...
def index():
    ...
    return f"<html><head>{CSS}{JS}</head><body><div class='hero'><div class='logo-font'>{logo}</div><input type='text' id='q' class='wide-search' onkeypress='check(event, \"text\")' placeholder='検索またはURL'></div></body></html>"

@app.route('/s/<q>/<t>')
def results(q, t):
    query = de(q)
    if query.startswith(('http://', 'https://')): return redirect(f'/p/{ob(query)}')
    html = f"<html><head>{CSS}{JS}</head><body><div class='header'><a href='/' style='text-decoration:none; font-size:20px; font-weight:bold;'><span style='color:#4285f4'>G</span></a><input type='text' id='q' value='{query}' onkeypress='check(event, \"{t}\")'></div><div class='container'>"
    try:
        with DDGS() as ddgs:
            for r in list(ddgs.text(query, max_results=10)):
